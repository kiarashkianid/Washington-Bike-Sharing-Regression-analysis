...
initialFitted<-lm (sqrt(cnt) ~ temp +I(temp^2)+ log(windspeed+1) + sqrt(hum)+ I(hum^2) + I(season == 1) + I(workingday == 1)+ hum*windspeed, data = hourData)
...
numericalData <- hourData[, c("temp", "hum", "windspeed","workingday","season","cnt")]
...
plot( log(hourData$windspeed+1), residuals,
...
plot(sqrt(hourData$hum), residuals,
...
Since VIF values are less than 5 , the model predictors seem to be acceptable 
...
-we check residuals against each predictor to identify specific non-linear relationships. Residual vs Temperature seem to be problematic as it show a pattern that indicates the current model is not explaining the relation among predictors. 
- we will add a quadratic term I(temp^2) since the residual vs temperature plots pattern seems to follow a quadratic function.
- we will add I(hum^2) since the residual vs humidity plot seems to follow a non linear pattern.
- we will also add humidityXwindspeed , and humidityXtemp interaction terms since the correlation matrix shows that these predictors have strong relationships with one another.

resulting model after the transformation show some improvement with regard to linearity assumption but both heteroscedasticity and normality assumption still is needed to be addressed. 
...
Now we need to address the heteroscedasticity present. We first try to transform response only (using log) the decision is due to the fact that log transformation is used for cases where variance of residual increase with the number of fitted value. however after applying the log transformation we see that the residuals go from a reverse funnel to a funnel pattern (decreasing variance). this suggest that the log transformation is over correcting the heteroscedasticity. we will try sq-rt transformation on the response. which slightly improve our model. since transforming only the response does not address the non constant variance present in the residuals we will try to transform our predictors. 
- the plot of residuals vs temperature seems to satisfy the homoscedasticity so we dont transform temperature
- we transform windspeed due to a funnel pattern present, we use a log transformation(windspeed+1), +1 is for not taking log(0).
- we transform humidity by sqrt transformation 
